...
export default function AdminCollectionsPage() {
  ...
  const fetchItems = useCallback((p: number, q: string) => {
    ...
    api.get<PaginatedResponse<CollectionItem>>(`/api/v1/admin/collections?${params}`)
      .then((res) => {
        setItems(res.items || []);
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
      ...
      <div className="relative mb-6">
        ...
        <input type="text" value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} placeholder="Search by name, description, or ID..." className="block w-full rounded-xl border border-slate-200 bg-white pl-10 pr-4 py-2.5 text-sm text-slate-900 placeholder-slate-400 transition-colors focus:border-sky-400 focus:outline-none focus:ring-2 focus:ring-sky-100" />
      </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50/50">
                  ...
                  <th className="px-4 py-3 text-left font-medium text-slate-500 hidden md:table-cell">Description</th>
                ...
                {items.map((item) => (
                  <React.Fragment key={item.id}>
                    <tr onClick={() => setExpandedId(expandedId === item.id ? null : item.id)} className="border-b border-slate-50 transition-colors hover:bg-slate-50/50 cursor-pointer">
                      <td className="px-4 py-3 font-mono text-xs font-medium text-slate-700">{item.collection_id}</td>
                      <td className="px-4 py-3 font-medium text-slate-900 max-w-[200px] truncate">{item.name}</td>
                      <td className="px-4 py-3 text-slate-500 hidden md:table-cell max-w-[250px] truncate">{item.description || "—"}</td>
                            ...
                            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                              {item.hero_image_url && (
                                <div className="rounded-xl border border-slate-100 bg-white p-4 shadow-sm">
                                  <p className="mb-2 text-xs font-semibold uppercase tracking-wider text-slate-400">Hero Image</p>
                                  <img src={item.hero_image_url} alt={item.name} className="h-32 w-full rounded-lg object-cover" onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
                                </div>
                              )}
                              <div className="rounded-xl border border-slate-100 bg-white p-4 shadow-sm">
                                ...
                                <div className="space-y-1.5">
                                  <div className="flex items-center justify-between gap-2">
                                    <span className="text-xs text-slate-500">Collection ID</span>
                                    <span className="text-xs font-medium text-slate-700 font-mono">{item.collection_id}</span>
                                  </div>
                                  <div className="flex items-center justify-between gap-2">
                                  ...
                                  {item.card_image_url && (
                                    <div className="flex items-center justify-between gap-2">
                                      <span className="text-xs text-slate-500">Card Image</span>
                                      <span className="text-xs font-medium text-slate-700 font-mono truncate max-w-[150px]">{item.card_image_url}</span>
                                    </div>
                                  )}
                                  <div className="flex items-center justify-between gap-2">
