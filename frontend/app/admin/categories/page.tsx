...
export default function AdminCategoriesPage() {
  ...
  const handleSync = useCallback(async () => {
    ...
    try {
      const res = await api.post<Record<string, unknown>>("/api/v1/admin/categories/sync");
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">Manage Headout categories synced from API</p>
        </div>
        <button
          onClick={handleSync}
          disabled={syncModal.running}
          className="flex items-center gap-2 rounded-xl bg-sky-500 px-4 py-2.5 text-sm font-medium text-white transition-colors hover:bg-sky-600 disabled:opacity-50"
        >
          {syncModal.running ? <Loader2 className="h-4 w-4 animate-spin" /> : <RefreshCw className="h-4 w-4" />}
          {syncModal.running ? "Syncing..." : "Sync Categories"}
        </button>
      </div>
                ...
                {items.map((item) => (
                  <React.Fragment key={item.id}>
                    <tr onClick={() => setExpandedId(expandedId === item.id ? null : item.id)} className="border-b border-slate-50 transition-colors hover:bg-slate-50/50 cursor-pointer">
                      <td className="px-4 py-3 font-mono text-xs font-medium text-slate-700">{item.category_id}</td>
                              ...
                              <div className="rounded-xl border border-slate-100 bg-white p-4 shadow-sm">
                                ...
                                <div className="space-y-1.5">
                                  <div className="flex items-center justify-between gap-2">
                                    ...
                                    <span className="text-xs font-medium text-slate-700 font-mono">{item.category_id}</span>
                                  </div>
                                  <div className="flex items-center justify-between gap-2">
